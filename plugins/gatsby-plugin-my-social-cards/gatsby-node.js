...
exports.createPages = (
  { cache, graphql, actions: { createPage } },
  {
    query,
    queryToPages,
  ...
  return graphql(query).then((result) => {
    ...
    dimensions.forEach((dimension) => {
      existingSocialCards.push(
        ...glob.sync(
          join(process.cwd(), imageFolder, `*${dimension.suffix}.png`),
        ),
      );
    });
    ...
    if (paths.length > 0) {
      console.info(
        `Successfully created ${paths.length} social card pages. First one is ${paths[0].path}`,
      );
    } else {
...
exports.onCreateDevServer = (
  ...
  cache.get(CACHE_KEY).then(async (socialCardPaths) => {
    ...
    for (const socialCardPath of socialCardPaths) {
      ...
      const destinationFile = join(process.cwd(), imageFolder, `${path}.png`);
